...
public class AdminUserController {
    ...
    public AdminUserController(AppUserRepository users, UserGroupRepository groups, PasswordEncoder passwords, AuditService audit) {
    ...
    public UserView update(@PathVariable UUID id, @Valid @RequestBody UpdateUser request, Authentication auth) {
        ...
        user.update(request.displayName(), request.role(), request.status(), request.monthlyPageQuota(), request.quotaExempt());
    ...
    public record UpdateUser(@NotBlank @Size(max=120) String displayName, @NotNull Role role, @NotNull UserStatus status,
    ...
    public record UserView(UUID id, String email, String displayName, Role role, UserStatus status,
                           Integer monthlyPageQuota, boolean quotaExempt, Instant createdAt) {
        static UserView from(AppUser u) { return new UserView(u.getId(), u.getEmail(), u.getDisplayName(), u.getRole(), u.getStatus(), u.getMonthlyPageQuota(), u.isQuotaExempt(), u.getCreatedAt()); }
    }
}
