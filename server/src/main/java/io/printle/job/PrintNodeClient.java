...
public class PrintNodeClient {
    ...
    public PrintNodeClient(ObjectMapper json, PrintleProperties properties) {
        ...
        this.token = properties.printNodeToken();
    }
    ...
    public Submission submit(UUID key, String queue, Path file, String title, String user, int copies,
        ...
        return submit(key, queue, file, title, user, copies, color, duplex, null);
    }
    public Submission submit(UUID key, String queue, Path file, String title, String user, int copies,
                             ColorMode color, DuplexMode duplex, String pageSet) {
        ...
        try {
            ...
            var builder = HttpRequest.newBuilder(uri).timeout(Duration.ofSeconds(30))
                ...
                .header("X-Print-Node-Token", token)
            ...
            if (pageSet != null) builder.header("X-Print-Page-Set", pageSet);
            var request = builder.POST(HttpRequest.BodyPublishers.ofFile(file)).build();
    ...
    public IppStatus status(int cupsJobId) {
        try {
            var request = HttpRequest.newBuilder(URI.create(baseUrl + "/jobs/" + cupsJobId))
                .timeout(Duration.ofSeconds(5)).header("X-Print-Node-Token", token).GET().build();
            var response = http.send(request, HttpResponse.BodyHandlers.ofString());
            if (response.statusCode() / 100 != 2) throw new IOExceptionResponse(response.body());
    ...
    public List<PrinterProfile> printers() {
        try {
            var request = HttpRequest.newBuilder(URI.create(baseUrl + "/printers")).timeout(Duration.ofSeconds(5))
                .header("X-Print-Node-Token", token).GET().build();
            var response = http.send(request, HttpResponse.BodyHandlers.ofString());
            if (response.statusCode() / 100 != 2) throw new IOExceptionResponse(response.body());
            return json.readerForListOf(PrinterProfile.class).readValue(response.body());
        } catch (Exception e) { throw new IllegalStateException("Could not read print-node printers", e); }
    }

    public void cancel(int cupsJobId) {
        try {
            var request = HttpRequest.newBuilder(URI.create(baseUrl + "/jobs/" + cupsJobId + "/cancel"))
                .timeout(Duration.ofSeconds(8)).header("X-Print-Node-Token", token).POST(HttpRequest.BodyPublishers.noBody()).build();
            var response = http.send(request, HttpResponse.BodyHandlers.ofString());
            if (response.statusCode() / 100 != 2) throw new IOExceptionResponse(response.body());
        } catch (Exception e) { throw new ResponseStatusException(HttpStatus.BAD_GATEWAY, "CUPS could not cancel the job", e); }
    }
    ...
    public record PrinterProfile(String queue, String name, String location, String status, boolean enabled,
                                 boolean color, boolean duplex, List<String> media, List<String> reasons, Device device) {}
    public record Device(String transport, String vendorId, String productId, String serial, String deviceId) {}
