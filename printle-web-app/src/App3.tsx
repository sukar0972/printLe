...
export default function App() {
  ...
  // Persistent Settings (Mocking localStorage)
  ...
  const [history, setHistory] = useState<PrintJob[]>([
    ...
    { id: '2', fileName: 'invoice_1024.png', fileSize: '450 KB', date: new Date(Date.now() - 172800000).toLocaleString(), status: 'failed', printer: 'Office Jet Pro' },
  ]);
  ...
  const handlePrint = async (stage: 'odd' | 'even' | 'single' = 'single') => {
    ...
    try {
      ...
      if (stage !== 'single') formData.append('duplex', stage);

      // Perform the actual upload to the configured server URL
      ...
      const newJob: PrintJob = {
        id: generateId(),
        fileName: file.name + (manualDuplex ? ' (Duplex)' : ''),
      ...
      // Poll for status or simulate completion for now
      setTimeout(() => {
      ...
      setIsUploading(false);
      setUploadStage(null);
      alert(`Failed to send print job. Check console and server URL: ${settings.serverUrl}`);
    }
  };
  ...
  return (
    <div className={`min-h-screen flex flex-col bg-slate-50 dark:bg-slate-900 transition-colors duration-300 font-sans text-slate-900 dark:text-slate-100 ${settings.theme}`}>
            ...
            >
              ...
              {isUploading ? (
                ...
                <div className="flex flex-col items-center space-y-4 relative z-10">
                   <div className="bg-blue-100 dark:bg-slate-700 p-4 rounded-xl relative">
                      {file.type.includes('image') ? <ImageIcon className="text-blue-600 dark:text-blue-400" size={40} /> : <FileText className="text-blue-600 dark:text-blue-400" size={40} />}
                      <button 
                   ...
                   <div className="space-y-1">
                      <p className="font-semibold text-lg truncate max-w-[200px] sm:max-w-xs">{file.name}</p>
                      <p className="text-sm text-slate-500">{formatBytes(file.size)}</p>
                   </div>
                   
                   {/* Duplex Toggle */}
                   <div 
                     onClick={(e) => { e.stopPropagation(); setManualDuplex(!manualDuplex); }}
                     className={`flex items-center gap-2 px-4 py-2 rounded-lg border transition-colors cursor-pointer select-none
                       ${manualDuplex 
                         ? 'border-blue-500 bg-blue-50 dark:bg-blue-900/30 text-blue-700 dark:text-blue-300' 
                         : 'border-slate-200 dark:border-slate-600 hover:bg-slate-50 dark:hover:bg-slate-700'}`}
                   >
                      <div className={`w-4 h-4 rounded border flex items-center justify-center
                        ${manualDuplex ? 'bg-blue-500 border-blue-500' : 'border-slate-400'}`}>
                        {manualDuplex && <CheckCircle size={12} className="text-white" />}
                      </div>
                      <Layers size={16} />
                      <span className="text-sm font-medium">Manual Double-Sided</span>
                   </div>

                   <div className="flex items-center gap-2 pt-4">
                      <button 
                        onClick={(e) => { e.stopPropagation(); startPrintProcess(); }}
                        className="bg-blue-600 hover:bg-blue-700 text-white font-semibold py-3 px-8 rounded-full shadow-lg shadow-blue-500/30 transition-transform active:scale-95 flex items-center gap-2"
                      >
                        <Printer size={18} />
                        Print Now
                      </button>
        ...
        {activeTab === 'settings' && (
          <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
            ...
            <div className="bg-white dark:bg-slate-800 rounded-xl shadow-sm border border-slate-200 dark:border-slate-700 overflow-hidden">
              <div className="p-4 border-b border-slate-100 dark:border-slate-700 bg-slate-50 dark:bg-slate-800/50">
                <h3 className="font-semibold flex items-center gap-2">
                  <Settings size={18} /> Print Defaults
                </h3>
              </div>
              <div className="p-4 space-y-4">
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    ...
                    <select 
                      ...
                      className="w-full p-2 rounded-lg border border-slate-300 dark:border-slate-600 bg-white dark:bg-slate-700 outline-none"
                    >
                      <option>A4</option>
                  ...
                  <div>
                    ...
                    <select 
                      ...
                      onChange={(e) => setSettings({...settings, defaultColor: e.target.value as any})}
                      className="w-full p-2 rounded-lg border border-slate-300 dark:border-slate-600 bg-white dark:bg-slate-700 outline-none"
                    >
                      <option>Color</option>
      ...
      {/* Mobile Bottom Navigation (Hidden on large screens, or kept for consistency if desired) */}
