...
test('renders a dashboard preview with sample jobs', async () => {
  ...
  expect(screen.getByRole('tab', { name: '1 Pills' })).toHaveAttribute('aria-selected', 'true')
})

test('cycles a status mark from the hash', async () => {
  window.location.hash = '#preview/pair'
  render(<App />)
  expect(await screen.findByRole('heading', { name: 'Queue' })).toBeInTheDocument()
  expect(screen.getByRole('tab', { name: '7 Pair' })).toHaveAttribute('aria-selected', 'true')
})
